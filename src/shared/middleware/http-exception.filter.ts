    ...
    private logger = this.loggingService.getLogger('http-exception');
        ...
        if (typeof excResponse !== 'object' || !excResponse.hasOwnProperty('success')) {
            ...
            excResponse = new exc.BadRequestException({
                ...
                serviceName: excResponse?.serviceName ? excResponse.serviceName : configService.get(EEnvKey.APP_NAME),
            }).getResponse();
        } else {
            excResponse.serviceName = excResponse?.serviceName
                ? excResponse.serviceName
                : configService.get(EEnvKey.APP_NAME);
        }
