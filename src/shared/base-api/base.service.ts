    ...
    implements IBaseService<MainEntity>
{
    ...
    async getEntityById<T extends BaseEntity>(_id: EntityId): Promise<T> {
        const entity = await this.mainRepository.findOne(_id);
        const normalize = await this.normalize([entity]);
        return normalize[0];
    }

    async getEntityByConditions(filters: FindConditions<MainEntity>) {
        const entity = await this.mainRepository.findOne(filters);
        const normalize = await this.normalize([entity]);
        return normalize[0];
    }

    async listEntities(filters: FindConditions<MainEntity>) {
    ...
    async deleteByConditions(filters: FindConditions<MainEntity>): Promise<DeleteResult> {
